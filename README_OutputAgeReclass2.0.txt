Title:			README_OutputAgeReclass2.0
Project:		LANDIS-II Landscape Change Model
Project Component:	Extension-Output-Age-Reclassification
Component Deposition:	https://github.com/LANDIS-II-Foundation/Extension-Output-Age-Reclassification
Author:			LANDIS-II Foundation
Origin Date:		23 Apr 2017
Final Date:		


Welcome to the source code repository for Extension-Output-Age-Reclassification, a 
LANDIS-II output extension.

Extension-Output-Age-Reclassification uses cohort information at each site to (re)classify every active
site into a forest type defined by the user. A fuzzy algorithm is used whereby a dominance value is
calculated for each species using a Reclass Coefficient with value (0.0 <= RC <= 1.0). The Reclass Coefficient 
is user-defined.

Each forest type has an associated group of species and species dominance values are summed 
for each forest type to which they are assigned. Note that some species may have a negative dominance for 
a given forest type, as determined by the user. Ultimately, a site is assigned (reclassified) to the forest
type with the highest total dominance value.
